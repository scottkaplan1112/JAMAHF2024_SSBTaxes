...
PlaceboDataGenFun <- function(ATTData, zip3, PreTreatmentMeans_Data, LongDF, treatment_time, ATTAllTaxed_DF){
  ...
  # ##Determine "Actual" and "Predicted" data 
  # LongDF_AllTaxed_City <- filter(LongDF_AllTaxed, store_zip3 == zip3) %>%
  #   dplyr::select(-treated)
  # LongDF_Placebos <- filter(LongDF, store_zip3 != zip3)
  # LongDF_Placebos <- rbind(LongDF_Placebos, LongDF_AllTaxed_City)
  # LongDF_Placebos <- mutate(LongDF_Placebos, Time = calendar_time - treatment_time - 1)
  # 
  # PlaceboData <- left_join(PlaceboData, LongDF_Placebos,
  #                             by= c("Level" = "store_zip3",
  #                                   "Time" = "Time"))
  # 
  # PlaceboData <- mutate(PlaceboData, TotalOz_zipcode_predicted = TotalOz_zipcode + Estimate)
  ...
  # PlaceboData <- mutate(PlaceboData, PrePeriod = ifelse(Time < 0, 1, 0))
  # MSPE <- group_by(PlaceboData, PrePeriod, Level) %>%
  #   summarise(MSPE = sqrt(mean((TotalOz_zipcode - TotalOz_zipcode_predicted)^2, na.rm = TRUE)),
  #             MSPE_Normalized = sqrt(mean((TotalOz_zipcode - TotalOz_zipcode_predicted)^2, na.rm = TRUE))/PretreatmentMean) %>%
  #   distinct()
  # 
  # PlaceboData <- left_join(PlaceboData, MSPE,
  #                             by = c("Level" = "Level",
  #                                    "PrePeriod" = "PrePeriod")) 
  # 
  # ##Filtering out pre-period normalized MSPEs 2x or more larger than treated city pre-period normalized MSPE
  # PrePeriod_Normalized_MSPE <- filter(MSPE, PrePeriod == 1 & Level == zip3) %>% distinct()
  # PrePeriod_Normalized_MSPE <- PrePeriod_Normalized_MSPE$MSPE_Normalized
  # MSPE_Pruned <- filter(MSPE, PrePeriod == 1 & MSPE_Normalized < 2*PrePeriod_Normalized_MSPE)
  # 
  # ##Only keeping valid placebos
  # PlaceboData_Pruned <- filter(PlaceboData, Level %in% MSPE_Pruned$Level)
  # 
  # ##Taking a sample of 100 placebos
  # Placebo_Zips <- unique(PlaceboData_Pruned$Level)
  # Sample_Placebos <- c(sample(Placebo_Zips, 100), zip3)
  # PlaceboData_Pruned_Sample <- filter(PlaceboData_Pruned, Level %in% Sample_Placebos) %>% ungroup()
  # 
  # ##Calculating the ranking
  # MSPE_Ranking <- filter(PlaceboData_Pruned_Sample, PrePeriod == 0) %>%
  #   distinct(Level, MSPE_Normalized) %>%
  #   mutate(MSPE_rank = rank(desc(MSPE_Normalized)))
  # 
  # ##Calculating p-value
  # pval <- filter(MSPE_Ranking, Level == zip3) %>%
  #   distinct(MSPE_rank/100) %>% as.numeric
...
PlaceboDataGenFun_Average <- function(ATTData, PreTreatmentMeans_Data, LongDF){
  ...
  # ##Calculating the ranking
  # MSPE_Ranking <- filter(PlaceboData_Pruned_Sample, PrePeriod == 0) %>%
  #   distinct(Placebo_ID, MSPE_Normalized) %>%
  #   mutate(MSPE_rank = rank(desc(MSPE_Normalized)))
  # 
  # ##Calculating p-value
  # pval <- filter(MSPE_Ranking, Placebo_ID == "Average") %>%
...
##Conformal Inference Plots
Plot_Fun <- function(BiasedSCMObject, city_name, treated_zip, type){
  
  # BiasedSCMObject <- SF_BiasedSCM
  # city_name <- "San Francisco"
  # treated_zip <- "941"
  # type <- "Consumption"
  
  # #######Synthetic Control#########
  # SCData <- BiasedSCMObject$SC_summary$att
  # MeanEstimate <- filter(SCData, Time >= 0) %>% summarise(mean = mean(Estimate))
  # MeanEstimate <- MeanEstimate[1,1]
  # MeanLowerBound <- filter(SCData, Time >= 0) %>% summarise(mean = mean(lower_bound))
  # MeanLowerBound <- MeanLowerBound[1,1]
  # MeanUpperBound <- filter(SCData, Time >= 0) %>% summarise(mean = mean(upper_bound))
  # MeanUpperBound <- MeanUpperBound[1,1]
  # Meanpval <- filter(SCData, Time >= 0) %>% summarise(mean = mean(p_val))
  # Meanpval <- Meanpval[1,1]
  # 
  # syn_plot_formatted <- ggplot(data = SCData) + ggplot2::geom_hline(yintercept = 0, size = 1.1, color = "red", linetype = "solid") + 
  #   ggplot2::geom_vline(xintercept = 0, size = 1.1, color = "red", linetype = "dashed") + 
  #   ggplot2::geom_line(aes(x=Time, y=100*(Estimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)), size = 1.5, alpha = 1, color = "black") + 
  #   #ggplot2::geom_line(aes(x=Time, y=100*(lower_bound/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)), size = 1, alpha = 1, color = "grey", linetype = "dashed") + 
  #   #ggplot2::geom_line(aes(x=Time, y=100*(upper_bound/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)), size = 1, alpha = 1, color = "grey", linetype = "dashed") + 
  #   #ggplot2::geom_point(aes(x=Time, y=Estimate), color = "#b41e7c") + 
  #   annotate("text", color = "red", size = 8, x = 12, 
  #            y = max(100*(SCData$Estimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)+10), label = "Start of Tax") +
  #   geom_segment(aes(x = 7.5, y = max(100*(SCData$Estimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)+10), 
  #                    xend = 1, yend = max(100*(SCData$Estimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)+10),),
  #                color = "red", size = 1, arrow = arrow(length = unit(0.5, "cm"))) + 
  #   geom_segment(aes(x = 0, y = 100*(MeanEstimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`), 
  #                    xend = 24, yend = 100*(MeanEstimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`),),
  #                color = "grey", size = 1) + 
  #   annotate("label", color = "black", size = 10, x = -24, 
  #            y = 100*(MeanEstimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)-5, 
  #            label = paste0("Effect Estimate:\n", round(100*(MeanEstimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`), 1), "%")) +
  #   geom_segment(aes(x = -17, y = 100*(MeanEstimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)-5,
  #                    xend = 2, yend = 100*(MeanEstimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)-2,),
  #                color = "black", size = 1, arrow = arrow(length = unit(0.5, "cm"))) + 
  #   ggplot2::labs(color = "", linetype = "", y = "Scaled Treatment Effect (in Percent)", x = "Event-Time (Months)", 
  #                 title = paste0(city_name, ": ", treated_zip)) + 
  #   ggplot2::theme_minimal() +
  #   scale_x_continuous(breaks = seq(min(BiasedSCMObject$SC_summary$att$Time)+2, max(BiasedSCMObject$SC_summary$att$Time)+2, by = 12)) +
  #   scale_y_continuous(labels = comma) +
  #   theme_bw() + 
  #   theme(legend.title=element_blank(), 
  #         plot.title = element_text(hjust = 0.5, size = 30),
  ...
  # 
  # if(Zips_All == 0){
  #   ggsave(paste0("Figures/Results/", city_name, "/Bias Corrected SCM/SyntheticControl_Zip9Only.png"), width = 40, height = 25, units = "cm")
  # }
  # 
  # if(Zips_All == 1){
  #   ggsave(paste0("Figures/Results/", city_name, "/Bias Corrected SCM/SyntheticControl.png"), width = 40, height = 25, units = "cm")
  # }
  # 
  # #######Augmented Synthetic Control#########
  # SCData <- BiasedSCMObject$SC_summary_augmented$att
  # MeanEstimate <- filter(SCData, Time >= 0) %>% summarise(mean = mean(Estimate))
  # MeanEstimate <- MeanEstimate[1,1]
  # MeanLowerBound <- filter(SCData, Time >= 0) %>% summarise(mean = mean(lower_bound))
  # MeanLowerBound <- MeanLowerBound[1,1]
  ...
  # Meanpval <- Meanpval[1,1]
  # 
  # asyn_plot_formatted <- ggplot(data = SCData) + ggplot2::geom_hline(yintercept = 0, size = 1.1, color = "red", linetype = "solid") + 
  #   ggplot2::geom_vline(xintercept = 0, size = 1.1, color = "red", linetype = "dashed") + 
  #   ggplot2::geom_line(aes(x=Time, y=100*(Estimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)), size = 1.5, alpha = 1, color = "black") + 
  #   #ggplot2::geom_line(aes(x=Time, y=100*(lower_bound/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)), size = 1, alpha = 1, color = "grey", linetype = "dashed") + 
  #   #ggplot2::geom_line(aes(x=Time, y=100*(upper_bound/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)), size = 1, alpha = 1, color = "grey", linetype = "dashed") + 
  ...
  # 
  # if(Zips_All == 0){
  #   ggsave(paste0("Figures/Results/", city_name, "/Bias Corrected SCM/SyntheticControl_Augmented_Zip9Only.png"), width = 40, height = 25, units = "cm")
  # }
  # 
  # if(Zips_All == 1){
  #   ggsave(paste0("Figures/Results/", city_name, "/Bias Corrected SCM/SyntheticControl_Augmented.png"), width = 40, height = 25, units = "cm")
  # }
  
  #######Augmented Synthetic Control w/ Covariates#########
  SCData <- BiasedSCMObject$SC_summary_augmented_covariates$att
  MeanEstimate <- filter(SCData, Time >= 0) %>% summarise(mean = mean(Estimate))
  MeanEstimate <- MeanEstimate[1,1]
  MeanLowerBound <- filter(SCData, Time >= 0) %>% summarise(mean = mean(lower_bound))
  MeanLowerBound <- MeanLowerBound[1,1]
  ...
  covsyn_plot_formatted <- ggplot(data = SCData) + ggplot2::geom_hline(yintercept = 0, size = 1.1, color = "red", linetype = "solid") + 
    ggplot2::geom_vline(xintercept = 0, size = 1.1, color = "red", linetype = "dashed") + 
    ggplot2::geom_line(aes(x=Time, y=100*(Estimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)), size = 1.5, alpha = 1, color = "black") + 
    ggplot2::geom_line(aes(x=Time, y=100*(lower_bound/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)), size = 1, alpha = 1, color = "grey", linetype = "dashed") + 
    ggplot2::geom_line(aes(x=Time, y=100*(upper_bound/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)), size = 1, alpha = 1, color = "grey", linetype = "dashed") + 
    ...
    annotate("label", color = "black", size = 10, x = -24, 
             y = 100*(MeanEstimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)-5, 
             label = paste0("Effect Estimate:\n", round(100*(MeanEstimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`), 1), "% (", 
                            round(100*(MeanLowerBound/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`), 1), ", ",
                            round(100*(MeanUpperBound/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`), 1), "), p = ",
                            round(Meanpval, 2))) +
    geom_segment(aes(x = -10.5, y = 100*(MeanEstimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)-5,
                     xend = 2, yend = 100*(MeanEstimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)-2,),
                 color = "black", size = 1, arrow = arrow(length = unit(0.5, "cm"))) + 
    ggplot2::labs(color = "", linetype = "", y = "Scaled Treatment Effect (in Percent)", x = "Event-Time (Months)", 
                  title = paste0(city_name, ": ", treated_zip)) + 
    ...
    theme(legend.title=element_blank(), 
          ...
          axis.title.y = element_text(size = 32, margin = margin(t = 0, r = 20, b = 0, l = 0)),
          axis.title.x = element_text(size = 32, margin = margin(t = 20, r = 0, b = 0, l = 0))) +
  covsyn_plot_formatted
  
  ggsave(paste0("Figures/Results/Multisynth Results/", type, "/Urbanicity/multisynth_conformalinference_", city_name, ".png"), width = 40, height = 25, units = "cm")
  
  
  # ##COMMENTING OUT RESIDUAL PLOT TO SAVE TIME ON RUNS
  #######Residualized Augmented Synthetic Control#########
  # covsyn_resid_plot <- plot(covsyn_resid)
  # covsyn_resid_plot_formatted <- covsyn_resid_plot + ggplot2::geom_hline(yintercept = 0, color = "black", linetype = 2) + 
  # ggplot2::geom_vline(xintercept = 0, color = "black") + 
  #   ggplot2::geom_line(size = 1, alpha = 0.75, color = "#b41e7c") + ggplot2::geom_point(color = "#b41e7c") + 
  #   ggplot2::labs(color = "", linetype = "", y = "Ounces Sold (Zip Code by Month)", x = "Event-Time (Months)", 
  #                 title = paste0("Difference in the Synthetic Control and Observed Treatment Zip Code: ", city_name)) + 
  #   ggplot2::theme_minimal() +
  #   scale_x_continuous(breaks = seq(min(BiasedSCMObject$SC_summary$att$Time)+2, max(BiasedSCMObject$SC_summary$att$Time)+2, by = 12)) +
  #   scale_y_continuous(labels = comma) +
  #   theme_bw() + 
  ...
  #         axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)),
  #         axis.title.x = element_text(margin = margin(t = 20, r = 0, b = 0, l = 0))) 
  # covsyn_resid_plot_formatted
  # 
  # if(Zips_All == 0){
  # ggsave(paste0("Figures/Results/", city_name, "/Bias Corrected SCM/SyntheticControl_Augmented_Covariates_Residualized_Zip9Only.png"), width = 40, height = 25, units = "cm")
  # }
  # 
  # if(Zips_All == 1){
  # ggsave(paste0("Figures/Results/", city_name, "/Bias Corrected SCM/SyntheticControl_Augmented_Covariates_Residualized.png"), width = 40, height = 25, units = "cm")
  # }
  
  
  # #######Synthetic Control w/ Unit FEs#########
  # SCData <- BiasedSCMObject$SC_summary_UnitFEs$att
  # MeanEstimate <- filter(SCData, Time >= 0) %>% summarise(mean = mean(Estimate))
  # MeanEstimate <- MeanEstimate[1,1]
  # MeanLowerBound <- filter(SCData, Time >= 0) %>% summarise(mean = mean(lower_bound))
  # MeanLowerBound <- MeanLowerBound[1,1]
  ...
  # Meanpval <- Meanpval[1,1]
  # 
  # desyn_plot_formatted <- ggplot(data = SCData) + ggplot2::geom_hline(yintercept = 0, size = 1.1, color = "red", linetype = "solid") + 
  #   ggplot2::geom_vline(xintercept = 0, size = 1.1, color = "red", linetype = "dashed") + 
  #   ggplot2::geom_line(aes(x=Time, y=100*(Estimate/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)), size = 1.5, alpha = 1, color = "black") + 
  #   #ggplot2::geom_line(aes(x=Time, y=100*(lower_bound/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)), size = 1, alpha = 1, color = "grey", linetype = "dashed") + 
  #   #ggplot2::geom_line(aes(x=Time, y=100*(upper_bound/BiasedSCMObject$PreTreatMean_ATE_DF$`Pretreatment Mean`)), size = 1, alpha = 1, color = "grey", linetype = "dashed") + 
  ...
  #         axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)),
  #         axis.title.x = element_text(margin = margin(t = 20, r = 0, b = 0, l = 0))) 
  # desyn_plot_formatted
  # 
  # 
  # if(Zips_All == 0){
  #   ggsave(paste0("Figures/Results/", city_name, "/Bias Corrected SCM/SyntheticControl_UnitFE_Zip9Only.png"), width = 40, height = 25, units = "cm")
  # }
  # 
  # if(Zips_All == 1){
  #   ggsave(paste0("Figures/Results/", city_name, "/Bias Corrected SCM/SyntheticControl_UnitFE.png"), width = 40, height = 25, units = "cm")
  # }
  
  return(covsyn_plot_formatted)
}
